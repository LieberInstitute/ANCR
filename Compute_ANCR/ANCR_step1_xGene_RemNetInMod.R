###############################################################################
## This code provides results for Figure 3CD.
...
## computed, considering different proportions of top module MAGMA genes, after 
## removing different proportions of top MAGMA genes in the network,
...
modulewise.fn = function(currentModule , allModules, 
  ...
  ## Figure 3CD ###subTMP is data for genes in module, removing genes based on network wide MAGMA rank ( considering magma_quant cutoff)
  subTMP   <- df_small_quantcrop %>% filter(.data[[Mcol]] %in% currentModule) %>%
    {if (length(KWITcol)>0) {
      filter(., is.finite(.data[[KWITcol]]))
    } else {
      .
  ...
  subTMP = subTMP[complete.cases(subTMP),]
  if(nrow(subTMP) < 10) return(NULL)
  ...
  ## Figure 3CD ### otherTMP_og is data for genes outside module, NOT removing genes based on network wide MAGMA rank (not considering magma_quant cutoff).
  currentKMEcol_og = paste0(networkName,".KME",currentModule) %>% .[. %in% colnames(df_small_nocrop)]
  otherTMP_og <- df_small_nocrop %>% filter(!.data[[Mcol]] %in% currentModule) %>%
   {if (length(currentKMEcol_og)>0) {
     filter(., is.finite(.data[[currentKMEcol_og]]))
   } else {
     .
   }
   }
  ...
  allResults = map_dfr(c(zVars),~{
    ...
    otherTMP_var = otherTMP_og[!is.na(otherTMP_og[currentVar]),]
    ...
    median_H <- allQuant %>% set_names(paste0("median_H_Q",.)) %>% map_dfc(~{
      ...
      ## Figure 3CD ### For Figure 3CD, this value is computed considering the previous network wide gene removal based on magma_quant, thus subTMP is used
      residuals(confoundMod_nocrop)[rownames(subTMP),currentVar]%>%
        sort(decreasing = TRUE)                %>%
        head(ceiling(nrow(subTMP)/whichQuant)) %>%
        median()
...
zVars_select = zVars[grepl("SCZ.PGC3.kbp35.10.ZSTAT|AD.|UC.|CD.|ALS.|BIP.|MDD.|ASD.|ADHD.|ASD.|RA.|PD.", zVars)] 
zVars_select = zVars[grepl("PD.", zVars)] 
